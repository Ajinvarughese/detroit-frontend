...
const actionableStatus = ["REQUESTED", "PENDING", "APPROVED"];
...
const LoansTable = () => {
    ...
    useEffect(() => {
        const fetchAllLoans = async () => {
            try {
                ...
                const sortedLoans = loanRes.data
    ...
    const handleDecision = async (id, loan, decision) => {
        if (loan.interestRate == null || loan.amount == null || loan.durationMonths == null) {
        ...
        try {
            ...
            setLoans((prev) =>
                prev.map((l) =>
                    l.id === id ? { ...l, status: loanRes.data.status } : l
                )
            );
        } catch (error) {
                            ...
                            <React.Fragment key={loan?.id}>
                                ...
                                >
                                    ...
                                    <td className={`px-4 py-3 ${
                                        loan?.status === 'APPROVED' ? 'text-[#4ade80]' :
                                            loan?.status === 'DISBURSED' ? 'text-[#9D00FF]' :
...

                                                {
                                                    ...
                                                    <div className="flex flex-col gap-3 space-y-3">
                                                        ...
                                                        <div className="mt-3 flex gap-5">
                                                            <button
                                                                className="cursor-pointer bg-purple-500 text-white px-4 py-2 rounded transition-all duration-300 hover:bg-purple-600"
                                                                onClick={() => handleDecision(loan?.id, loan, 'DISBURSED')}
                                                            >
                                                                Disbursed
                                                            </button>
                                                        </div>
